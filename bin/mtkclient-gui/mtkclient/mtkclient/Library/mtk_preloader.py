...
class Preloader(metaclass=LogBase):
    ...
    def init(self, maxtries=None, display=True):
        ...
        if not display:
            self.info("状态: 正在等待 PreLoader VCOM 端口，请重启设备一次，再重新进入 brom 模式。")
        else:
            self.info("状态: 正在等待 PreLoader VCOM 端口，请连接设备。")
        res = False
        tries = 0
    ...
    def setreg_disablewatchdogtimer(self, hwcode):
        ...
        if not res:
            self.error("收到错误的 SetReg_DisableWatchDogTimer 回应")
            return False
        if hwcode == 0x6592:
