...
for /f "tokens=2 delims= " %%i in ('findstr "ro.vivo.product.solution" log\adbgetprop.log') do set cpuid=%%i
if %cpuid%==[QCOM] goto QCOM
if %cpuid%==[MTK] goto MTK
:MTK
set LKs=black
fastboot.exe oem LKs 1>nul 2>log\fbgetvar.log
for /f "tokens=2 delims= " %%i in ('findstr "LKs=" log\fbgetvar.log') do set LKs=%%i
if %LKs%==0 set abdev="n"
if %LKs%==1 echo.%rootrec-p27%& goto EXIT
if %LKs%==black goto OTHER
:QCOM
set unlocked=black
fastboot.exe oem device-info 1>nul 2>log\fbgetvar.log
for /f "tokens=2 delims= " %%i in ('findstr "Device unlocked:" log\fbgetvar.log') do set unlocked=%%i
if %unlocked%==true set abdev="n"
if %unlocked%==false echo.%rootrec-p27%& goto EXITING
if %unlocked%==black goto OTHER
:OTHER
for /f "tokens=2 delims= " %%i in ('findstr "ro.boot.flash.locked" log\adbgetprop.log') do set other=%%i
if %other%==[1] echo.%rootrec-p27%& goto EXITING
if %other%==[0] set abdev="n"
...
if "%choice%"=="6" echo.installed>log\drvinst.log & start https://syxz.lanzoub.com/i0t0V063oggb
...
fastboot.exe oem LKs 1>nul 2>log\fbgetvar.log
for /f "tokens=2 delims= " %%i in ('findstr "LKs=" log\fbgetvar.log') do set LKs=%%i
if %LKs%==0 set goto ROOT-9
if %LKs%==1 echo.%rootrec-p27%& goto EXITING
if %LKs%==black goto OTHER
:QCOM
set unlocked=black
fastboot.exe oem device-info 1>nul 2>log\fbgetvar.log
for /f "tokens=2 delims= " %%i in ('findstr "Device unlocked:" log\fbgetvar.log') do set unlocked=%%i
if %unlocked%==true goto ROOT-9
if %unlocked%==false echo.%rootrec-p27%& goto EXITING
if %unlocked%==black goto OTHER
:OTHER
for /f "tokens=2 delims= " %%i in ('findstr "ro.boot.flash.locked" log\adbgetprop.log') do set other=%%i
if %other%==[1] echo.%rootrec-p27%& goto EXITING
if %other%==[0] goto ROOT-9
:ROOT-9
ECHOC {0F}%root-p32%{\n}
set abdev=n
...
ECHO.%root-x9-p5%
7z.exe x "X9.7z" -y 1>nul
del X9.7z 1>nul 2>nul
...
ECHOC {0E}  %unlock-p13%{\n}
pause>nul
if exist log\model_new.txt goto CHK
aria2c.exe --max-concurrent-downloads=16 --max-connection-per-server=16 --split=16 --dir=log %dlsource%/model_new.txt | find "download completed." 1>nul 2>nul
if not "%errorlevel%"=="0" ECHOC {0C}%choosedev-p3%{\n}& pause>nul & goto CHOOSEPLAN-TEMPORARY
find "found" "log\model_new.txt" | find ":500," 1>nul 2>nul
if "%errorlevel%"=="0" ECHOC {0C}%choosedev-p4%{\n}& pause>nul & goto CHOOSEPLAN-TEMPORARY
...
fastboot.exe oem LKs 1>nul 2>log\fbgetvar.log
for /f "tokens=2 delims= " %%i in ('findstr "LKs=" log\fbgetvar.log') do set LKs=%%i
if %LKs%==0 echo.%unlock-p53%{\n}& goto START
if %LKs%==1 goto CHOOSEPLAN
if %LKs%==black goto OTHER
:QCOM
set unlocked=black
ECHOC {0E}- [%unlock-p51%]{\n}
fastboot.exe oem device-info 1>nul 2>log\fbgetvar.log
for /f "tokens=2 delims= " %%i in ('findstr "Device unlocked:" log\fbgetvar.log') do set unlocked=%%i
if %unlocked%==true echo.%unlock-p53%& goto START
if %unlocked%==false goto CHOOSEPLAN
if %unlocked%==black goto OTHER
:OTHER
for /f "tokens=2 delims= " %%i in ('findstr "ro.boot.flash.locked" log\adbgetprop.log') do set other=%%i
if %other%==[1] echo.%unlock-p60%& goto CHOOSEPLAN
if %other%==[0] echo.%unlock-p53%& goto START
:CHOOSEPLAN
for /f "tokens=2 delims= " %%i in ('findstr "ro.product.device" log\adbgetprop.log') do set model=%%i
for /f "tokens=4 delims= " %%a in ('findstr "%model%" log\model_new.txt') do set unlockplan=%%a
echo.%unlock-p61%%unlockplan%
...
::aria2c.exe --max-concurrent-downloads=16 --max-connection-per-server=16 --split=16 https://gitee.com/mouzei/viqoo/attach_files/1082081/download/mtkclient-gui.7z 1>nul 2>nul
::if not exist mtkclient-gui.7z ECHOC {0C}  下载失败！按任意键重试...{\n}& pause>nul & goto UNLOCK-MTK
